...
protected:
    ...
    inline void BindFunc(const SQChar* name, void* method, size_t methodSize, SQFUNCTION func, bool staticVar = false, int nparamscheck = 0, SQChar* typemask = NULL) {
        sq_pushobject(vm, GetObject());
		...
		//2017/02/03: à»â∫ÇÕé©ï™Ç≈í«â¡ÇµÇΩïîï™Ç∆à¯êîÇæÇÊ
    ...
    inline void BindFunc(const SQInteger index, void* method, size_t methodSize, SQFUNCTION func, bool staticVar = false, int nparamscheck = 0, SQChar* typemask = NULL) {
        sq_pushobject(vm, GetObject());
        sq_pushinteger(vm, index);
