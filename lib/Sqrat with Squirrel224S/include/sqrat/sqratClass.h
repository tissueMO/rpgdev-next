...
public:
    ...
    */
    ...
    template<class F>
	Class& Func(const SQChar* name, F method, int nparamscheck, SQChar* typemask) {
		//2017/02/03: å„ÇÎÇQÇ¬ÇÃà¯êîÇí«â¡ÇµÇΩÉoÉCÉìÉhä÷êîÇé©ï™Ç≈í«â¡ÇµÇΩÇÊ
		BindFunc(name, &method, sizeof(method), SqMemberFunc(method), false, nparamscheck, typemask);
		return *this;
	}
	template<class F>
    ...
    template<class F>
	Class& GlobalFunc(const SQChar* name, F method, int nparamscheck, SQChar* typemask) {
		BindFunc(name, &method, sizeof(method), SqMemberGlobalFunc(method), false, nparamscheck, typemask);
		return *this;
	}
	template<class F>
    ...
    template<class F>
	Class& StaticFunc(const SQChar* name, F method, int nparamscheck, SQChar* typemask) {
		BindFunc(name, &method, sizeof(method), SqGlobalFunc(method), false, nparamscheck, typemask);
		return *this;
	}
	template<class F>
