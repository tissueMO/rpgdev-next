...
namespace Editor.Module {
	...
	public class FastDiff {
		...
		private void SearchSnake(int k) {
			...
			while((posA < this.dataA.Length) && (posB < this.dataB.Length) && this.isSame(posA, posB)) {
				posA++;
				posB++;
			}
		...
		private class CommonSubsequence {
			...
			public override string ToString() {
				return "Length:" + this.Length + ", A:" + this.StartA.ToString() + ", B:" + this.StartB.ToString() + ", Next:" + ((this.Next == null) ? "null" : "exist");
			}
		}
	}
}
